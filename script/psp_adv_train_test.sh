#sh script/psp_adv_train_test.sh
...
cp script/psp_adv_train_test.sh tool_train/train_at_psp.py ${config} ${exp_dir}
export PYTHONPATH=./
...
# ------------------------TEST------------------------
num_epoch=50
cp tool_test/voc2012/test_voc_psp_at.py ${exp_dir}
#Clean
...
#CosPGD3
$PYTHON -u tool_test/voc2012/test_voc_psp_at.py \
  --config=${config}  --attack=${attack} --at_iter=${at_iter} --test_attack cospgd --attack_iter=3 --num_epoch=${num_epoch} --train_num=${num}\
  2>&1 | tee -a ${result_dir}/test-${num}.log
#CosPGD5
$PYTHON -u tool_test/voc2012/test_voc_psp_at.py \
